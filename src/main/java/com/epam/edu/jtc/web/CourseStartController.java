...
public class CourseStartController {
    ...
    @RequestMapping(value = {"/courses/{coursesID}/start"}, method = RequestMethod.GET)
    public String getCourseStartGet(@PathVariable String coursesID,
                                    ModelMap model) {
        ...
        if ((userRoles != null)) {
            if (!userRoles.contains("LECTURER")) {
                ...
                model.addAttribute("startForbidden", ERROR_ACTION_FORBIDDEN);
            }
        }
        if (isLong(coursesID)) {
            ...
            if (coursesService.findCourseByCourseID(courseID) == null) {
                ...
                if ((course.getCoursesState().equals("Ready")))
    ...
    @RequestMapping(value = {"/courses/{coursesID}/start"}, method = RequestMethod.POST)
    public String getCourseStartPost(@PathVariable String coursesID,
                                     ModelMap model) {
        logger.debug("Start check: user is login? And get his name.");
        String userNameAuth = getAuthUser();
        logger.info("User come with " + userNameAuth + " name.");
        ...
        String courseState = "In progress";
